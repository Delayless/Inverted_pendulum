...
//		DataScope_Get_Channel_Data( 0 , 10);
		Send_Count = DataScope_Data_Generate(2);	//发送两个通道的数据
		for( i = 0 ; i < Send_Count; i++) 
		{
			while((USART1->SR&0X40)==0);	//TC发送完成标志位，没发完就等待
			//'$'  帧头
			//帧尾是字节数，也就是说有帧头加帧尾再加上中间的数据
...
void DataScope(void)
{   
		float try_Angle = 102.3;
		float try_Encoder = 52.7;
		DataScope_Get_Channel_Data( try_Angle, 1 );      
		DataScope_Get_Channel_Data( try_Encoder, 2 );         
		//DataScope_Get_Channel_Data( 0, 3 );              
//		DataScope_Get_Channel_Data( 0 , 4 );   
//		DataScope_Get_Channel_Data(0, 5 ); //用您要显示的数据替换0就行了
//		DataScope_Get_Channel_Data(0 , 6 );//用您要显示的数据替换0就行了
